...
fun LedgerScreen(vm: LedgerViewModel = hiltViewModel()) {
    ...
    LazyColumn(
        ...
        // ── Header ──
        item {
            Text("My Capital Ledger", fontSize = 22.sp, fontWeight = FontWeight.Bold,
                color = Color(0xFF0F172A))
            Text("${state.orgName} · ${state.memberName} (${state.memberId})",
        ...
        item {
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp),
                ...
                LedgerStatCard("My Capital", fmtC(state.totalCapital),
                    "verified contributions", Color(0xFF15803D), Color(0xFFF0FDF4), Modifier.weight(1f))
                LedgerStatCard("Total Profit", fmtC(state.totalProfit),
                    "all distributions", Color(0xFF1D4ED8), Color(0xFFEFF6FF), Modifier.weight(1f))
            }
        }

        // ── Section title ──
        item {
            Text("Payment History", fontWeight = FontWeight.Bold, fontSize = 14.sp,
                color = Color(0xFF0F172A), modifier = Modifier.padding(bottom = 10.dp, top = 4.dp))
        }
        ...
        if (state.entries.isEmpty()) {
            item {
                Column(
                    ...
                    Text("No payments yet", fontWeight = FontWeight.SemiBold, fontSize = 15.sp)
                    Text("Your payments will appear here once submitted.",
        ...
        Column(Modifier.padding(14.dp, 12.dp)) {
            ...
            Row(
                ...
                Column(Modifier.weight(1f)) {
                    Text(payTypeLabel(entry.type), fontWeight = FontWeight.Bold, fontSize = 13.sp,
                        color = payTypeColor(entry.type))
                    Text(entry.date, fontSize = 11.sp, color = Color(0xFF94A3B8),
                        modifier = Modifier.padding(top = 2.dp))
                }
                Column(horizontalAlignment = Alignment.End) {
                    Text(fmtC(entry.amount), fontWeight = FontWeight.ExtraBold, fontSize = 15.sp,
                        color = Color(0xFF0F172A))
                    // Status badge
                    val (sBg, sFg) = when {
                    ...
                    Surface(color = sBg, shape = RoundedCornerShape(99.dp),
                        ...
                        Text(entry.status, fontSize = 10.sp, fontWeight = FontWeight.Bold,
            ...
            Row(
                ...
                Surface(color = Color(0xFFF1F5F9), shape = RoundedCornerShape(6.dp)) {
                    Text(entry.method, fontSize = 11.sp, color = Color(0xFF475569),
                ...
                if (entry.gatewayFee > 0) {
                    Surface(color = Color(0xFFFEF3C7), shape = RoundedCornerShape(6.dp)) {
                        Text("Fee: ${fmtC(entry.gatewayFee)}", fontSize = 11.sp,
                            color = Color(0xFF92400E), modifier = Modifier.padding(7.dp, 3.dp))
                    }
                }
                ...
                if (entry.penaltyPaid > 0) {
                    Surface(color = Color(0xFFFEE2E2), shape = RoundedCornerShape(6.dp)) {
                        Text("Late: ${fmtC(entry.penaltyPaid)}", fontSize = 11.sp,
                            color = Color(0xFFDC2626), modifier = Modifier.padding(7.dp, 3.dp))
                    }
                }
            }

            // Paid months (for monthly entries)
            ...
            // Running capital total (only for verified contributions)
            if (isVerified && entry.isContribution && entry.runningTotal > 0) {
                ...
                Row(
                    ...
                    Text(fmtC(entry.netAmount), fontSize = 11.sp,
                ...
                Row(
                    ...
                    Text(fmtC(entry.runningTotal), fontSize = 12.sp,
...
fun LedgerStatCard(
    ...
    Surface(color = bg, shape = RoundedCornerShape(12.dp),
        ...
        Column(Modifier.padding(14.dp)) {
            ...
            Text(sub, fontSize = 11.sp, color = Color(0xFF94A3B8),
                modifier = Modifier.padding(top = 3.dp))
        }
    }
}